...
/**
 * Resource for handling requests for Kronecker Product calculations.
 * See the MatrixApplication class for URI mappings
 * 
 * @author Jonathan Cohen
 */
public class MatrixKroneckerProductResource extends ServerResource
{
	private static Logger logger = MatrixLogger.getInstance();
	
	
    /**
     * Handle POST request to perform the Kronecker product of a list of matrices
     * @param entity XMl formatted matrix list
     * @return XML representation of the Kronecker product
     * @throws ResourceException
     */
	@Post
    public Representation matrixKroneckerProduct(Representation entity) throws ResourceException
    {
		if(entity == null)
		{
        ...
        try
        {
        	...
        	matrixList = MatrixParamParser.
              getKroneckerProductParamsFromDomNode( rep.getDocument().getDocumentElement() );
            ...
            for(int i = 1; i < matrixList.size(); i++){
            	...
            	productMatrix = new NamedRealMatrix( MatrixUtils.getKroneckerProduct(productMatrix, nextMatrix) );
            }
